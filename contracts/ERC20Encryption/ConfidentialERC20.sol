...
abstract contract ConfidentialERC20 is IConfidentialERC20, IERC20Errors, TFHEErrors {
    ...
    mapping(address account => euint256 balance) internal _balances;

    /// @notice A mapping of the form mapping(account => mapping(spender => allowance)).
    mapping(address account => mapping(address spender => euint256 allowance)) internal _allowances;
    ...
    function transfer(address to, einput encryptedAmount, bytes calldata inputProof) public virtual returns (bool) {
        transfer(to, TFHE.asEuint256(encryptedAmount, inputProof));
        return true;
    }
     ...
     */
    function _unsafeMint(address account, uint256 amount) internal virtual {
     ...
     *      by the function calling it.
     */
    function _unsafeMintNoEvent(address account, uint256 amount) internal virtual {
        euint256 newBalanceAccount = TFHE.add(_balances[account], amount);
    ...
    function _transferNoEvent(address from, address to, euint256 amount, ebool isTransferable) internal virtual {
        if (from == address(0)) {
        ...
        euint256 transferValue = TFHE.select(isTransferable, amount, TFHE.asEuint256(0));
        euint256 newBalanceTo = TFHE.add(_balances[to], transferValue);
        _balances[to] = newBalanceTo;
        TFHE.allowThis(newBalanceTo);
        TFHE.allow(newBalanceTo, to);
        euint256 newBalanceFrom = TFHE.sub(_balances[from], transferValue);
    ...
    function _isSenderAllowedForAmount(euint256 amount) internal view virtual {
